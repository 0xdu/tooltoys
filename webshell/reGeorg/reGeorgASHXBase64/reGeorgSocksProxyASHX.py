...
class session(Thread):
    ...
    def parseSocks5(self, sock):
        ...
        if cmd == "\x02":  # BIND
            ...
            self.cookie, self.processid = self.setupRemoteSession(gethostbyname(target), targetPort)
            if self.cookie:
    ...
    def setupRemoteSession(self, target, port):
        ...
        session_id = '-1'
        process_id = '-1'
        if DEBUG: print "[+] setupRemoteSession: POST ", self.connectString + "?iidd=1&1=%s&2=%d" % (
            target, port)
        if DEBUG: print "[+] setupRemoteSession: request_headers: ", repr(headers)
        response = self.requests_session.post(self.connectString + "?iidd=1&1=%s&2=%d" % (target, port), allow_redirects=False, verify=False)
        ...
        if response.status_code == 200:
            status = response.headers["cors-status"]
            if status == "1":
                if 'X-SESSIONID' in response.headers:
                    session_id = response.headers['X-SESSIONID']
                if 'X-PROCESSID' in response.headers:
                    process_id = response.headers['X-PROCESSID']
                log.debug('[%s:%d] [%s:%s] SetupRemoteSession - OK' % (self.target, self.port, session_id, process_id))
            else:
                if response.headers["cors-error"] is not None:
                    log.error("[%s:%d] SetupRemoteSession - ERROR - %s" % (
                        self.target, self.port, response.headers["cors-error"]))
        else:
            log.error("[%s:%d] SetupRemoteSession - Code: %d | Message: %s | Data: %s" % (
                self.target, self.port, response.status_code, response.headers["cors-error"], response.content))
    ...
    def closeRemoteSession(self):
        ...
        if self.closed:
            log.debug("[%s:%d] [%s:%s] CloseRemoteSession - Connection already closed" % (
        ...
        while num_retry < MAX_RETRY:
            ...
            params = ""
            if DEBUG: log.debug(
                "[+] closeRemoteSession: POST " + self.connectString + "?iidd=2&processid=" + self.processid + "&sessionid=" + self.cookie)
            if DEBUG: log.debug("[+] closeRemoteSession: request_headers: " + repr(headers))
            response = self.requests_session.post(
                self.connectString + "?iidd=2&processid=" + self.processid + "&sessionid=" + self.cookie, allow_redirects=False, verify=False)
        ...
        while not self.closed:
            try:
                ...
                while num_retry < MAX_RETRY:
                    ...
                    if DEBUG: log.debug(
                        "[+] reader: POST " + self.connectString + "?iidd=4&processid=" + self.processid + "&sessionid=" + self.cookie)
                    if DEBUG: log.debug("[+] reader: request_headers: " + repr(headers))
                    response = self.requests_session.post(
                        self.connectString + "?iidd=4&processid=" + self.processid + "&sessionid=" + self.cookie, allow_redirects=False, verify=False)
                    ...
                    if response.status_code == 200:
                        status = response.headers["cors-status"]
                        if status == "1":
                            # print 'Received:', response.content[:128]
                            ...
                            log.debug("[%s:%d] [%s:%s] Reader - RETRY" % (
                                self.target, self.port, self.cookie, self.processid))
                            continue
                        else:
                            data = None
                ...
                if data is None:
                    ...
                    log.debug(
                        "[%s:%d] [%s:%s] Reader - Data is None" % (self.target, self.port, self.cookie, self.processid))
                    break
                if len(data) == 0:
                    sleep(0.1)
                    continue
                transferLog.info("[%s:%d] [%s:%s] Reader - <<<< [%d] | %r" % (
                    self.target, self.port, self.cookie, self.processid, len(data), data[:128]))
    ...
    def writer(self):
        ...
        while not self.closed:
            try:
                ...
                # print 'Going to send:', data[:128]
                if not data:
                    log.debug(
                        "[%s:%d] [%s:%s] Writer - No more data" % (self.target, self.port, self.cookie, self.processid))
                    break
                num_retry = 0
                while num_retry < MAX_RETRY:
                    if DEBUG: log.debug("[+] Try to send data, num_retry = %d" % num_retry)
                    if DEBUG: log.debug(
                        "[+] writer: POST " + self.connectString + "?iidd=3&processid=" + self.processid + "&sessionid=" + self.cookie)
                    if DEBUG: log.debug("[+] writer: request_headers: " + repr(headers))
                    response = self.requests_session.post(
                        self.connectString + "?iidd=3&processid=" + self.processid + "&sessionid=" + self.cookie, data=base64.b64encode(data), allow_redirects=False, verify=False)
                    ...
                    if response.status_code == 200:
                        ...
                        if status == "1":
                            ...
                            transferLog.info("[%s:%d] [%s:%s] Writer - Retry: %d | >>>> [%d] | %r" % (
                                self.target, self.port, self.cookie, self.processid, num_retry, len(data), data[:128]))
                            break  # finish retry loop
                        elif status == "RETRY":
                            # retry another worker process
                            num_retry += 1
                            headers = {}
                            log.debug("[%s:%d] [%s:%s] Writer - RETRY" % (
                                self.target, self.port, self.cookie, self.processid))
                            continue
                        else:
                        ...
                        log.error(
                            "[%s:%d] [%s:%s] Writer - ERROR - Code: %d | Data: %s" % (
                                self.target, self.port, self.cookie, self.processid, response.status_code,
                                response.content))
                        break  # finish retry loop
            except timeout:
